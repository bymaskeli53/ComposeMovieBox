...
class MovieDaoTest {
    ...
    fun testDeleteMovie() =
        runBlocking {
            ...
            assertEquals(movies, emptyList<FavoriteMovieEntity>())
        }

    @Test
    fun testMoviesInOrder() = runBlocking {
        val movie = FavoriteMovieEntity(2, "Lord Of The Rings", "One rule them all", "lord.jpg")
        val movie2 = FavoriteMovieEntity(1, "Inception", "Best movie ever", "inception.jpg")


        movieDao.insertFavoriteMovie(movie)
        movieDao.insertFavoriteMovie(movie2)

        val movies = movieDao.getAllFavoriteMovies()

        assertEquals(movies[0], movie2)
        assertEquals(movies[1], movie)
